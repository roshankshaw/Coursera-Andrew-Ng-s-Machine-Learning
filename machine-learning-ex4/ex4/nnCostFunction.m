...
X=[ones(m,1) X];
a1=X;
z2=a1*Theta1';  %dimension m*s(2)
a2=sigmoid(z2);
a2=[ones(m,1) a2] ;  %dimension m*s(2)+1
z3=a2*Theta2';
a3=sigmoid(z3); %dimension m*s(3)
h=a3;%dimension s(3)*m
%cost function
J=0;
for k=1:num_labels
yk = y==k;
...
theta1_dup=Theta1(:,2:end);
theta2_dup=Theta2(:,2:end);
%regularizing cost function
J=J+(lambda/(2*m))*(sum(sum(theta1_dup.^2)) + sum(sum(theta2_dup.^2)));
